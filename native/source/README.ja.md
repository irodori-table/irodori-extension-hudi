...
このディレクトリは `irodori.hudi` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、エンジンの動作は `src/driver.rs` にあります。エンジン固有の接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動すべきです。コネクタのランタイム契約がデスクトップアプリに組み込まれるときに移行します。
